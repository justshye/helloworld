...
def trevorPageView(request):
    # return request object and specify page.
    return render(request, 'trevor.html')
...
def results(request, choice, gmat):
    print("*** Inside reults()")
    # load saved model
    with open(r'C:\Users\shye\PycharmProjects\COMP4949Lab-Lessons\helloworld\model_pkl', 'rb') as f:
    ...
    if response.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('message',
                                                kwargs={'msg': "You are registered.", 'title': "Success!"}, ))
    # Handle GET request.
    else:
        form = RegisterForm()
